...
class ServicePOExcel(models.TransientModel):
    _name = 'service.po.excel'
    _description = 'Service PO Report Excel'
    ...
    def tick_ok(self):
        ...
        worksheet1 = workbook.add_sheet('Service Purchase Order Report')
        ...
        serial_no = 1
        for datas in data:
            # worksheet1.write(rows, cnt, serial_no, design_7)
            if datas['po_num']:
            ...
            if datas['delivery_sch']:
                worksheet1.write_merge(rows, rows, 10, 11, datas['delivery_sch'].strftime('%d-%m-%Y'), design_2)
            else:
        ...
        self.write(
            {'summary_file': excel_file,
             'file_name': 'Service PO Report  - [ %s ].xls' % self.start_date.strftime('%d/%m/%Y'),
             'report_printed': True})
        fp.close()
        return {
            'view_mode': 'form',
            'res_id': self.id,
            'res_model': 'service.po.excel',
            'view_type': 'form',
