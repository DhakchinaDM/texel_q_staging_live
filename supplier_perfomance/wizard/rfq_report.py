...
class RfqReportExcel(models.TransientModel):
    _name = 'rfq.report.excel'
    _description = 'RFQ Report Excel'

    start_date = fields.Datetime(string="Start Date")
    end_date = fields.Datetime(string="End Date")
    purchase_orders = fields.Many2many('purchase.request', string='RFQ')
    ...
    def tick_ok(self):
        ...
        orders = self.env['purchase.request'].search(domain)
        data = []
        for val in orders:
            for i in val.request_ids:
                vendor_name = ", ".join([j.name for j in val.partner_id])
                var = {
                    'date': val.create_date,
                    'rfq_number': val.name,
                    'vendor_name': vendor_name,
                    'description': i.supplier_part_no,
                    'qty': i.req_qty,
                    'price': '-',
                    'status': dict(val._fields['state'].selection).get(val.state),
                }
                data.append(var)
        ...
        rows += 2
        worksheet1.write_merge(rows, rows, 0, 13, 'RFQ Report', design_5)
        rows += 2
        ...
        rows += 2
        worksheet1.write_merge(rows, rows, 0, 0, 'RFQ Date', design_5)
        worksheet1.write_merge(rows, rows, 1, 2, 'RFQ Number', design_5)
        worksheet1.write_merge(rows, rows, 3, 4, 'Vendor Name', design_5)
        worksheet1.write_merge(rows, rows, 5, 6, 'Description ', design_5)
        worksheet1.write_merge(rows, rows, 7, 9, 'Qty', design_5)
        worksheet1.write_merge(rows, rows, 10, 11, 'Price', design_5)
        worksheet1.write_merge(rows, rows, 12, 13, 'Status', design_5)
        rows += 1
        cnt = 0
        for datas in data:
            if datas['date']:
                worksheet1.write(rows, 0, datas['date'].strftime('%d-%m-%Y'), design_2)
            else:
                worksheet1.write(rows, 0, '-', design_1)
            if datas['rfq_number']:
                worksheet1.write_merge(rows, rows, 1, 2, datas['rfq_number'], design_2)
            else:
            ...
            if datas['description']:
                worksheet1.write_merge(rows, rows, 5, 6, datas['description'], design_2)
            else:
                worksheet1.write_merge(rows, rows, 5, 6, '-', design_1)
            if datas['qty']:
                worksheet1.write_merge(rows, rows, 7, 9, datas['qty'], design_2)
            else:
                worksheet1.write_merge(rows, rows, 7, 9, '0', design_1)
            if datas['price']:
                worksheet1.write_merge(rows, rows, 10, 11, datas['price'], design_3)
            else:
                worksheet1.write_merge(rows, rows, 10, 11, '-', design_1)
            if datas['status']:
                worksheet1.write_merge(rows, rows, 12, 13, datas['status'], design_3)
            else:
        ...
        self.write(
            {'summary_file': excel_file,
             'file_name': 'RFQ Report  - [ %s ].xls' % self.start_date.strftime('%d/%m/%Y'),
             'report_printed': True})
        fp.close()
        return {
            'name': 'RFQ Report',
