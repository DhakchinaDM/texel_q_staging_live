...
class PendingServicePOExcel(models.TransientModel):
    _name = 'pending.service.po.excel'
    ...
    def tick_ok(self):
        domain = [('create_date', '>=', self.start_date), ('create_date', '<=', self.end_date),
                  ('state', '=', 'purchase'), ('receipt_status', '=', 'partial')]
        ...
        for val in orders:
            var = {
                ...
                'texelq': val.order_line.product_id.default_code,
                'description': val.order_line.name,
                'qty_received': val.order_line.qty_received,
                'qty_pending': val.order_line.product_qty - val.order_line.qty_received
            }
            data.append(var)
        ...
        worksheet1 = workbook.add_sheet('Pending Service PO Report')
        ...
        self.write(
            {'summary_file': excel_file,
             'file_name': 'Pending Service PO Report  - [ %s ].xls' % self.start_date.strftime('%d/%m/%Y'),
             'report_printed': True})
        fp.close()
        return {
            'view_mode': 'form',
            'res_id': self.id,
            'res_model': 'pending.service.po.excel',
            'view_type': 'form',
